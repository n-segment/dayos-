...
현재 `ui.js`는 홈, 화면 꾸미기, 인증, 온보딩, 타임 테이블, 활동 저장, 레거시 히스토리까지 한 파일에 들어 있어 길이가 많이 커졌다. 다음 정리 세션에서는 기능을 새로 넣기보다 동작을 유지한 채 파일 경계를 먼저 나눈다.

### 권장 분리 순서

1. `storage.js`
   - localStorage / IndexedDB 키
   - 활동, 타임로그, 꾸미기 데이터 load/save
   - Firebase 로그인 여부에 따른 저장 분기
2. `time.js`
   - 날짜/시간 파싱과 포맷팅
   - 주간 범위 계산
   - 반복 규칙 계산
...
3. `home.js`
   - 홈 화면 시계와 날짜
   - 화면 꾸미기 모드
   - 배경 이미지/영상, 낙서, 메모, 링크 임베드
   - 음악 패널은 홈 꾸미기와 분리된 독립 기능으로 유지
4. `timetable.js`
   - 활동 목록
   - 계획/실행 블록
   - 주간 그리드 렌더링
...
6. `legacy-history.js`
   - 예전 타이머/기록/통계 화면
   - 실제로 계속 쓸지 결정하기 전까지는 분리만 하고 삭제하지 않는다.

### 정리 원칙

- 첫 번째 커밋은 동작 변경 없이 파일 이동과 import/export 정리만 한다.
- 이동 후 `node --check ui.js` 대신 각 JS 파일을 모두 문법 검사한다.
- 캐시 버전은 마지막에 한 번만 올린다.
- 전역 상태(`currentUser`, `acts`, `tlog`, `currentView`, `currentWeekStart`)가 어느 파일 소유인지 먼저 정한다.
- 분리 후에도 정적 호스팅에서 바로 열려야 하므로 번들러를 새로 도입하지 않는 방향을 우선한다.

## Empty State 점검

### 바로 넣으면 좋은 곳

- 활동 목록이 비었을 때: 사이드바에 `활동을 추가해 하루를 구성해보세요.` 같은 짧은 안내를 보여준다.
- 타임 테이블에 이번 주 블록이 하나도 없을 때: 그리드 위에 큰 빈 화면을 띄우지 말고, 활동 영역 아래에 `빈 시간을 눌러 계획을 추가할 수 있어요.` 정도로 작게 안내한다.
- 블록 상세의 할 일 목록이 비었을 때: 입력칸 위에 `이 시간에 할 일을 적어둘 수 있어요.` 같은 문구를 넣는다.

### 보류해도 되는 곳

- 홈 화면: 다이어리 표지처럼 비워두는 것이 의도라서 큰 empty state는 넣지 않는다.
- 화면 꾸미기: 도구 자체가 액션이라 별도 empty state보다 hover label과 버튼 상태가 더 중요하다.
- 음악 목록: 이미 비었을 때 `없음` 처리가 있다. 나중에 문구만 다듬으면 된다.
- 예전 타이머/히스토리 화면: 타이머 기능을 빼는 방향이라면 empty state를 추가하기보다 `legacy-history.js`로 분리한 뒤 유지 여부를 결정한다.

## v231

- 저녁/밤 홈 화면 팔레트에서 배경이 너무 밝게 보이지 않도록 홈 섹션 위에 어두운 딤 레이어를 추가했다.
- 낮 홈 화면은 기존 밝은 오버레이를 유지하고, 야간 팔레트에서만 딤을 적용한다.

## v232

- 홈 화면에 붙인 메모지를 드래그로 이동할 수 있게 정리했다.
- 메모 위치는 홈 화면 영역 안으로 제한하고, 이동 후 좌표를 저장한다.
- 기존 document mousemove 방식 대신 메모 카드 자체의 pointer drag로 바꿔 이벤트가 불필요하게 쌓이지 않게 했다.

## v233

- 메모지 이동을 pointer 이벤트와 mouse 이벤트 모두에서 처리하도록 보강했다.
- 드래그 중에는 document 레벨 move/up 리스너를 붙이고, 종료 시 제거해서 브라우저별 이벤트 전달 차이에도 안정적으로 움직이게 했다.
